...
out_image_path = sys.argv[3]

content_image = cv2.imread(content_image_path)
style_image = cv2.imread(style_image_path)
x, y, _ = content_image.shape
...
def compute_content_cost(a_C, a_G):
    ...
    J_content = (1.0 / (4.0 * n_H * n_W * n_C)) * tf.reduce_sum(tf.pow(a_C - a_G, 2))
...
def compute_layer_style_cost(a_S, a_G):
    ...
    J_style_layer = (1 / (4.0 * n_H * n_W * n_C * n_H * n_W * n_C)) * tf.reduce_sum(tf.pow(g_S - g_G, 2))
...
generated_image = cv2.resize(generated_image, (y, x))
save_image(out_image_path, generated_image)